...
const stubState = {
  ...
  rated_course:[],
  rated_course:[{id:0,score:5,expected:10,time:[{week_day:0,start_time:1020,end_time:1110},{week_day:2,start_time:1020,end_time:1110}],professor:'ABC',credit:4,location:'SNU'}],
  unrated_course:[{id:1,score:5,expected:10,time:[{week_day:0,start_time:1020,end_time:1110},{week_day:2,start_time:1020,end_time:1110}],professor:'ABC',credit:4,location:'SNU'}],
  ratedSearched: false,
...
const stubStateTrue = {
  user: { is_authenticated: true, timetable_main: 0 },
  rated_course:[],
  unrated_course:[{id:0,score:5,expected:10,time:[{week_day:0,start_time:1020,end_time:1110},{week_day:2,start_time:1020,end_time:1110}],professor:'ABC',credit:4,location:'SNU'}],
  ratedSearched: true,
...
jest.mock('../../../components/SearchBar/SearchBar', () => jest.fn((props) => (
  <div className="SearchBar">
    <button id="toggle-button" onClick={()=>props.onToggle()}/>
    <button id="change-button" onClick={()=>{props.onChange({target:{value:'A'}},'title');props.onChange({target:{value:'A'}},'classification');props.onChange({target:{value:'A'}},'department');props.onChange({target:{value:'A'}},'degree_program');props.onChange({target:{value:'A'}},'academic_year');props.onChange({target:{value:'A'}},'course_number');props.onChange({target:{value:'A'}},'lecture_number');props.onChange({target:{value:'A'}},'professor');props.onChange({target:{value:'A'}},'language');props.onChange({target:{value:0}},'min_credit');props.onChange({target:{value:4}},'max_credit');props.onChange({target:{value:0}},'min_score');props.onChange({target:{value:10}},'max_score');}}/>
    <button id="search-button" onClick={()=>props.onSearch()}/>
  </div>
)));
describe('<RecommendCourse />', () => {
  ...
  it('RecommendCourse render test', () => {
    let spySetRated = jest.spyOn(actionCreators, 'setRatedCourse')
    .mockImplementation(() => () => Promise.resolve(null));
    let spySetUnrated = jest.spyOn(actionCreators, 'setUnratedCourse')
    .mockImplementation(() => () => Promise.resolve(null));
    let spyGetRated = jest.spyOn(actionCreators, 'getRatedCourse')
    .mockImplementation(() => () => Promise.resolve(null));
    let spyGetUnrated = jest.spyOn(actionCreators, 'getUnratedCourse')
    .mockImplementation(() => () => Promise.resolve(null));
    let spyPutCoursepref = jest.spyOn(actionCreators, 'putCoursepref')
    .mockImplementation(() => () => Promise.resolve(null));
