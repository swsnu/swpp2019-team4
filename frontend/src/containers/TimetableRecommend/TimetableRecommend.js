...
class TimetableRecommend extends Component {
  ...
  render() {
    ...
    const detailList = [(
      <div key="recommendation-constraint-key">
      추천 받을 시간표가 만족할 조건을 입력합니다.
        <br />
        <br />
        <b>주 N일제: </b>
1주일에 수업이 있는 날 수가 N일 이하인 시간표만 추천합니다.
        <br />
        <br />
        <b>총 학점 범위: </b>
      시간표에 들어있는 과목들의 학점 수의 총합이 범위 안에 있는 시간표만 추천합니다.
        <br />
        <br />
        <b>전공 학점 범위: </b>
시간표에 들어있는 전공 과목의 학점 수의 총합이 범위 안에 있는 시간표만 추천합니다.
      </div>
    ),
    (
      <div key="recommendation-time-key">
      시간의 선호도를 표에 색칠하여 입력합니다.
        <br />
        <br />
      각각의 시간이 좋으면 하얀색, 싫으면 빨간색으로 칠하면 됩니다.
        <br />
        <br />
      오른쪽의 색깔 동그라미를 눌러서 색을 선택한 후, 표의 칸을 누르면 색칠됩니다.
        <br />
        <br />
좋은 평가를 내릴수록 추천 시간표에 해당 시간대의 과목이 들어갈 확률이 높아집니다.
      </div>
    ),
    (
      <div key="recommendation-course-key">
시간표에 넣을 과목을 선택하고 평가합니다.
        <br />
        <br />
      과목마다 있는 슬라이드바를 움직여서,
        <b>이 과목을 시간표에 넣고 싶은 정도</b>
를 평가합니다.
        <br />
        <br />
평가 탭에서는 지금까지 평가한 과목, 미평가 탭에서는 아직 평가하지 않은 과목을 보여줍니다.
        <br />
        <br />
        <b>추천 시간표에는 평가한 과목만 포함됩니다.</b>
        {' '}
즉, 평가하지 않은 과목은 추천 시간표에 들어가지 않습니다.
      </div>
    ),
    (
      <div key="recommendation-result-key">
      추천된 시간표를 조회합니다.
        <br />
        <br />
      시간표 저장 버튼을 누르면 내 시간표에 선택한 추천 시간표가 저장됩니다.
        <br />
        <br />
높게 평가했던 과목일수록 초록색, 낮게 평가했던 과목일수록 빨간색에 가까운 색으로 보여 줍니다.
      </div>
    ),
    ];
    for (let i = 0; i < progressLength; i += 1) {
