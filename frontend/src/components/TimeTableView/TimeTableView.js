...
/*
 * INPUT: props.timetable parsed as [{"day_index": 0, "start_time_index": 0, "end_time_index": 3, "course_name": "NAME OF EACH COURSE"}, {}, ...]
 * day_index is 0~5 integer. Monday is 0, Tuesday is 1, ... Saturday is 5
...
const TimeTableView=(props)=>{
    ...
    for(var i=0; i<props.courses.length; i++){
        var day=props.courses[i]["day_index"]+1;
        for(var j=props.courses[i]["start_time_index"]-15;j<props.courses[i]["end_time_index"]-15;j++){
    ...
    for(var i=0;i<tablestring.length;i++){
        var tablehtml_ith=[];
        for(var j=0;j<tablestring[i].length;j++){
