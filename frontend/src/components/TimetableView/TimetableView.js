 ...
 * 13 colors (excluding 3 grayscale colors)
 */

const colors = ['#FF851B', '#FFDC00', '#39CCCC', '#01FF70', '#F012BE', '#FF4136', '#7FDBFF',
  '#85144B', '#001F3F', '#3D9970', '#B10DC9', '#2ECC40', '#0074D9'];
...
const TimetableView = (props) => {
  ...
  if (props.courses !== undefined) {
    for (let i = 0; i < props.courses.length; i += 1) {
      for (let j = 0; j < props.courses[i].time.length; j += 1) {
        const segment = props.courses[i].time[j];
  ...
  for (let i = 0; i < 7; i += 1) {
    tablehtmlIth.push(<th key={i} height={heightunit}>{props.text ? tableHeaderString[i] : ''}</th>);
  }
          ...
          <td key={1000 * i + j} height={heightunit}>
            {
              coursesList[j][i].map(
                (course) => {
                  ...
                  if (props.link) {
                    // TODO: it has to be linked to the detailed course view.
                    return (
                      <a
